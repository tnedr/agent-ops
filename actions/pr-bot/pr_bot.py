...
def ci_checks_green(pr: int, timeout: int, force: bool = False) -> bool:
    if force:
        print("⚠️  --force flag set: skipping CI checks")
        return True
    
    start = time.time()
    while True:
        if time.time() - start > timeout:
            print("⏰ CI polling timed out.")
            return False
        try:
            ...
            if no_ci_output(err.args[2] if len(err.args) > 2 else str(err)):
                print("ℹ️  No CI configured – skipping checks.")
                return True
            else:
                print("❌ gh pr checks failed, will retry …")
        except json.JSONDecodeError:
            if no_ci_output(out):
                print("ℹ️  No CI configured – skipping checks.")
                return True
            ...
            if not checks:
                print("ℹ️  No CI checks present – proceed.")
                return True
...
def main() -> None:
    ...
    if pr_num is None:
        if not args.title or not args.message:
            print("❌ No existing PR found and --title/--message not provided.")
        ...
        try:
            ...
            print(f"❌ PR open error: {exc}")
            sys.exit(2)
        print(f"PR #{pr_num} opened.")
    else:
        print(f"🔄 Working with existing PR #{pr_num}")
    ...
    if args.force:
        merge_cmd += " --admin"
        print("⚠️  Using --force flag: merging with admin override (ignoring failed checks)")

    try:
        run(merge_cmd)
        print("✅ Merged & branch deleted.")
    except subprocess.CalledProcessError:
        print("❌ Auto‑merge failed – maybe conflict or insufficient permissions.")
        sys.exit(1)
