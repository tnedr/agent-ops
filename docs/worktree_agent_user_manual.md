# Worktree Agent User Manual

Complete guide for using `agent-tools` with multiple concurrent agents in any repository.
...
## Working with Multiple Concurrent Agents

`agent-tools` supports multiple agents working simultaneously in the same repository. Each agent has its own isolated worktree.

### Agent ID Detection

When multiple worktrees exist, you need to specify which agent to use:

**Method 1: Use `--agent` flag** (recommended for multiple agents):
```bash
...
**Method 2: Work from within worktree directory** (auto-detection):
```bash
cd .work/agent-xxxx
...
**Method 3: Legacy environment variable** (single agent only):
```bash
export AGENT_ID=agent-xxxx  # Linux/Mac
$env:AGENT_ID = "agent-xxxx"  # Windows PowerShell
python -m agt run "command"
```
...
```bash
# Agent 1: Start worktree
python -m agt start
# Output: AGENT_ID=agent-abc123

# Agent 2: Start worktree (in parallel)
python -m agt start
# Output: AGENT_ID=agent-def456

# Agent 1: Work in parallel
python -m agt run --agent agent-abc123 "echo 'Agent 1 work' > file1.txt"
python -m agt commit --agent agent-abc123 "feat: agent1 changes"

# Agent 2: Work in parallel
python -m agt run --agent agent-def456 "echo 'Agent 2 work' > file2.txt"
python -m agt commit --agent agent-def456 "feat: agent2 changes"

# Both agents can push independently
python -m agt push --agent agent-abc123
python -m agt push --agent agent-def456
```
...
python -m agt merge --agent agent-xxxx
